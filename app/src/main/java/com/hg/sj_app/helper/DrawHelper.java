...
public class DrawHelper {
    ...
    public static void drawFaceRect(Canvas canvas, DrawInfo drawInfo, int color, int faceRectThickness) {
        ...
        if (drawInfo.getName() == null) {
            ...
            String str = ((drawInfo.getState()==1?"查询中":(drawInfo.getState()==2?"未知":"")))+(drawInfo.getSex() == GenderInfo.MALE ? "男" : (drawInfo.getSex() == GenderInfo.FEMALE ? "女" : "未知性别"))
                    + ","
                    + (drawInfo.getAge() == AgeInfo.UNKNOWN_AGE ? "未知年龄" : drawInfo.getAge())
                    + ","
                    + (drawInfo.getLiveness() == LivenessInfo.ALIVE ? "活体" : (drawInfo.getLiveness() == LivenessInfo.NOT_ALIVE ? "非活体" : ""));
